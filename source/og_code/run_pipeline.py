...
from fill_template import (get_measurement_level_tags, get_paper_level_templates,
...
def run_pipeline(paper_filename, complementary_files=(), output_path="output.csv",
    ...
    # 3) measurement-level tags (persist back into each mapping file)
    ...
    for table in tables:
        ...
        if _identifier_column(table, mappings[tid]) is None:
            ...
            continue
        mappings[tid] = get_measurement_level_tags(mappings[tid], chunks)
        (Path(out_dir) / f"{tid}_mapping.json").write_text(
            json.dumps(mappings[tid], indent=2, ensure_ascii=False), encoding="utf-8")

    # 4) paper-level tags
    print("\n[4/5] Paper-level tags...")
    get_paper_level_templates(chunks, filepath=grouped_path)
